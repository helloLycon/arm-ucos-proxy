...
uint32_t ETH_Init(ETH_InitTypeDef* ETH_InitStruct, uint16_t PHYAddress)
{
  ...
  _eth_delay_(PHY_RESET_DELAY);
    //for(i=0;i<0x20;i++){
        testbyte82 = ETH_ReadPHYRegister(PHYAddress, 2);    //////
        //if(testbyte82!=0xffff){
         //   testbyte83++;
     //       break;
        //}
    //}
  ...
  {  
    linkstatus = (ETH_ReadPHYRegister(PHYAddress, PHY_BSR) & PHY_Linked_Status);
    if(linkstatus){  //is link
        #if 1
        ...
        do
        {
          ...
          //testbyte12 = ETH_ReadPHYRegister(PHYAddress, PHY_BSR);
          //testbyte13 = !(ETH_ReadPHYRegister(PHYAddress, PHY_BSR) & PHY_Linked_Status);
          
        } while (!(ETH_ReadPHYRegister(PHYAddress, PHY_BSR) & PHY_Linked_Status) && (timeout < PHY_READ_TO));
        //} while (testbyte13 && (timeout < PHY_READ_TO));
...
/**
  ...
  *         MAC MIIDR register value: Data read from the selected PHY register (correct read )
  */
uint16_t ETH_ReadPHYRegister_NonBlock(uint16_t PHYAddress, uint16_t PHYReg)
{
  uint32_t tmpreg = 0;     
__IO uint32_t timeout = 0;
  /* Check the parameters */
...
uint16_t ETH_PHY_read_data(void)
{
	return (uint16_t)(ETH->MACMIIDR);
}
...
void ETH_emac_set_duplex(int full_duplex)
{
	if (full_duplex) {
	    ETH->MACCR |= MAC_CFG_DM;
...
void ETH_emac_set_speed(int mbs_100)
{
	if (mbs_100) {
		ETH->MACCR |= MAC_CFG_FES;
	}
	else {
		ETH->MACCR &= ~MAC_CFG_FES;
	}
